...
import (
	...
	apimanagerevents "github.com/ekhvalov/bank-chat-service/tests/e2e/api/manager/events"
	apimanagerv1 "github.com/ekhvalov/bank-chat-service/tests/e2e/api/manager/v1"
)
...
func (ws *Workspace) SendMessageToChat(ctx context.Context, chatID types.ChatID, body string) error {
...
func (ws *Workspace) CloseChat(ctx context.Context, id types.ChatID) error {
	resp, err := ws.api.PostCloseChatWithResponse(ctx,
		&apimanagerv1.PostCloseChatParams{XRequestID: types.NewRequestID()},
		apimanagerv1.PostCloseChatJSONRequestBody{ChatId: id},
	)
	if err != nil {
		return fmt.Errorf("post request: %v", err)
	}
...
func (ws *Workspace) HandleEvent(_ context.Context, data []byte) error {
	...
	case apimanagerevents.ChatClosedEvent:
		...
		ws.setCanTakeMoreProblemsFlag(true)
	}
