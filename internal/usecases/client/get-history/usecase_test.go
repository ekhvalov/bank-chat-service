...
import (
	...
	gethistorymocks "github.com/ekhvalov/bank-chat-service/internal/usecases/client/get-history/mocks"
)
...
func (s *UseCaseSuite) TestGetClientChatMessages_Success_FirstPage() {
	...
	var actualCursor *messagesrepo.Cursor
	err = cursor.Decode(resp.NextCursor, &actualCursor)
	s.Require().NoError(err)
	cm := messagesrepo.NewCursorMatcher(*nextCursor)
	s.True(cm.Matches(actualCursor), fmt.Sprintf("cursors not matched: expected %q got '%v'", cm.String(), actualCursor))
	s.Require().Len(resp.Messages, messagesCount)
}
