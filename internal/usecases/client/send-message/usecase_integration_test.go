...
import (
	...
	sendmessagemocks "github.com/ekhvalov/bank-chat-service/internal/usecases/client/send-message/mocks"
)
...
func (s *UseCaseIntegrationSuite) SetupSuite() {
	...
	outBoxSvc, err := outbox.New(outbox.NewOptions(1, 10*time.Second, time.Minute, jobsRepo, s.Database, zap.NewNop()))
	s.Require().NoError(err)
