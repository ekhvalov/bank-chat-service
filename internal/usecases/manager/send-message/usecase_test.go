...
import (
	...
	sendmessagemocks "github.com/ekhvalov/bank-chat-service/internal/usecases/manager/send-message/mocks"
...
func (s *UseCaseSuite) TestOutboxServiceError() {
	...
	payload, err := jobpayload.Marshal(message.ID)
	s.Require().NoError(err)
	errOutbox := errors.New("outbox service error")
	s.outBoxSvc.EXPECT().Put(s.Ctx, sendmanagermessage.Name, payload, gomock.Any()).Return(types.JobIDNil, errOutbox)
...
func (s *UseCaseSuite) TestSuccess() {
	...
	payload, err := jobpayload.Marshal(message.ID)
	s.Require().NoError(err)
	s.outBoxSvc.EXPECT().Put(s.Ctx, sendmanagermessage.Name, payload, gomock.Any()).Return(types.NewJobID(), nil)
