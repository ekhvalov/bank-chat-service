...
func (s *ServiceSuite) TestPublishInDifferentUserStreams() {
	...
	s.T().Log("received hasEvent", receivedMsgs)
	s.Equal(expectedMsgs, receivedMsgs)
}

// readNewMessageEvents reads n hasEvent from the stream.
...
func BenchmarkPublishSubscribe(b *testing.B) {
	...
	defer cancel()
	ch, _ := stream.Subscribe(ctx, uid)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = stream.Publish(ctx, uid, msg)
		select {
		case <-ch:
		default:
		}
