...
func TestChatServiceSchema(t *testing.T) {
	...
	t.Run("cannot create problem without chat", func(t *testing.T) {
		_, err := client.Problem.Create().SetManagerID(types.NewUserID()).Save(ctx)
		t.Log(err)
		require.Error(t, err)
	})

	t.Run("do not accept zero ids", func(t *testing.T) {
		_, err := client.Chat.Create().SetClientID(types.UserID{}).Save(ctx)
		t.Log(err)
		require.Error(t, err)
	})
}
