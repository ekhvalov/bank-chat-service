...
import (
	...
	"github.com/ekhvalov/bank-chat-service/internal/store/gen/failedjob"
...
func (fjc *FailedJobCreate) check() error {
	if _, ok := fjc.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`gen: missing required field "FailedJob.name"`)}
	}
	if v, ok := fjc.mutation.Name(); ok {
		if err := failedjob.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`gen: validator failed for field "FailedJob.name": %w`, err)}
		}
	}
	if _, ok := fjc.mutation.Payload(); !ok {
		return &ValidationError{Name: "payload", err: errors.New(`gen: missing required field "FailedJob.payload"`)}
	}
	if v, ok := fjc.mutation.Payload(); ok {
		if err := failedjob.PayloadValidator(v); err != nil {
			return &ValidationError{Name: "payload", err: fmt.Errorf(`gen: validator failed for field "FailedJob.payload": %w`, err)}
		}
	}
	if _, ok := fjc.mutation.Reason(); !ok {
		return &ValidationError{Name: "reason", err: errors.New(`gen: missing required field "FailedJob.reason"`)}
	}
	if v, ok := fjc.mutation.Reason(); ok {
		if err := failedjob.ReasonValidator(v); err != nil {
			return &ValidationError{Name: "reason", err: fmt.Errorf(`gen: validator failed for field "FailedJob.reason": %w`, err)}
		}
	}
	if _, ok := fjc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`gen: missing required field "FailedJob.created_at"`)}
	}
	if v, ok := fjc.mutation.ID(); ok {
		if err := v.Validate(); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`gen: validator failed for field "FailedJob.id": %w`, err)}
		}
	}
	return nil
}
...
func (u *FailedJobUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("gen: missing options for FailedJobCreate.OnConflict")
	}
...
func (u *FailedJobUpsertOne) ID(ctx context.Context) (id types.FailedJobID, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("gen: FailedJobUpsertOne.ID is not supported by MySQL driver. Use FailedJobUpsertOne.Exec instead")
	}
...
func (u *FailedJobUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("gen: OnConflict was set for builder %d. Set it on the FailedJobCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("gen: missing options for FailedJobCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
