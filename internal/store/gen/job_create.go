...
func (jc *JobCreate) check() error {
	if _, ok := jc.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`gen: missing required field "Job.name"`)}
	}
	if v, ok := jc.mutation.Name(); ok {
		if err := job.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`gen: validator failed for field "Job.name": %w`, err)}
		}
	}
	if _, ok := jc.mutation.Payload(); !ok {
		return &ValidationError{Name: "payload", err: errors.New(`gen: missing required field "Job.payload"`)}
	}
	if v, ok := jc.mutation.Payload(); ok {
		if err := job.PayloadValidator(v); err != nil {
			return &ValidationError{Name: "payload", err: fmt.Errorf(`gen: validator failed for field "Job.payload": %w`, err)}
		}
	}
	if _, ok := jc.mutation.Attempts(); !ok {
		return &ValidationError{Name: "attempts", err: errors.New(`gen: missing required field "Job.attempts"`)}
	}
	...
	if _, ok := jc.mutation.AvailableAt(); !ok {
		return &ValidationError{Name: "available_at", err: errors.New(`gen: missing required field "Job.available_at"`)}
	}
	if _, ok := jc.mutation.ReservedUntil(); !ok {
		return &ValidationError{Name: "reserved_until", err: errors.New(`gen: missing required field "Job.reserved_until"`)}
	}
	if _, ok := jc.mutation.CreatedAt(); !ok {
	...
	if v, ok := jc.mutation.ID(); ok {
		if err := v.Validate(); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`gen: validator failed for field "Job.id": %w`, err)}
		}
	}
	return nil
}
...
func (u *JobUpsertOne) ID(ctx context.Context) (id types.JobID, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("gen: JobUpsertOne.ID is not supported by MySQL driver. Use JobUpsertOne.Exec instead")
	}
