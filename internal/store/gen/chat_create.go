...
func (cc *ChatCreate) check() error {
	if _, ok := cc.mutation.ClientID(); !ok {
		return &ValidationError{Name: "client_id", err: errors.New(`gen: missing required field "Chat.client_id"`)}
	}
	if v, ok := cc.mutation.ClientID(); ok {
		if err := v.Validate(); err != nil {
			return &ValidationError{Name: "client_id", err: fmt.Errorf(`gen: validator failed for field "Chat.client_id": %w`, err)}
		}
	}
	if _, ok := cc.mutation.CreatedAt(); !ok {
...
func (u *ChatUpsertOne) ID(ctx context.Context) (id types.ChatID, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("gen: ChatUpsertOne.ID is not supported by MySQL driver. Use ChatUpsertOne.Exec instead")
	}
