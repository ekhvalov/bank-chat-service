...
func (pc *ProblemCreate) check() error {
	if _, ok := pc.mutation.ChatID(); !ok {
		return &ValidationError{Name: "chat_id", err: errors.New(`gen: missing required field "Problem.chat_id"`)}
	}
	if v, ok := pc.mutation.ChatID(); ok {
		if err := v.Validate(); err != nil {
			return &ValidationError{Name: "chat_id", err: fmt.Errorf(`gen: validator failed for field "Problem.chat_id": %w`, err)}
		}
	}
	if v, ok := pc.mutation.ManagerID(); ok {
		if err := v.Validate(); err != nil {
			return &ValidationError{Name: "manager_id", err: fmt.Errorf(`gen: validator failed for field "Problem.manager_id": %w`, err)}
		}
	}
	if _, ok := pc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`gen: missing required field "Problem.created_at"`)}
	}
	if v, ok := pc.mutation.ID(); ok {
		if err := v.Validate(); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`gen: validator failed for field "Problem.id": %w`, err)}
		}
	}
	if _, ok := pc.mutation.ChatID(); !ok {
		return &ValidationError{Name: "chat", err: errors.New(`gen: missing required edge "Problem.chat"`)}
	}
	return nil
}
...
func (u *ProblemUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("gen: missing options for ProblemCreate.OnConflict")
	}
...
func (u *ProblemUpsertOne) ID(ctx context.Context) (id types.ProblemID, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("gen: ProblemUpsertOne.ID is not supported by MySQL driver. Use ProblemUpsertOne.Exec instead")
	}
...
func (u *ProblemUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("gen: OnConflict was set for builder %d. Set it on the ProblemCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("gen: missing options for ProblemCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
